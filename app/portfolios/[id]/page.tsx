...
const PortfolioDetail = () => {
  ...
  useEffect(() => {
    if (id) {
      setDataIndex(parseInt(id, 10)- 1 || 0);
    ...
    <div className="flex flex-col lg:flex-row items-center  gap-16 lg:gap-24 text-center lg:text-left px-6 p-5 md:pt-0 ">
      ...
      <h1 className="text-[36px] md:text-[44px]  text-[#1A906B] font-bold">
       Portfolio
        ...
        <h1 className="text-[24px] sm:text-[30] md:text-[36px] text-[#FF4500] font-semibold mb-4">{portfolioData[dataIndex].title}</h1>
...
</div> : <div className="flex max-w-full justify-center shadow-sm bg-[#f6f8f9fb] py-5 flex-1 overflow-y-auto overflow-hidden lg:max-h-[600px] rounded-lg px-5 md:px-10 ">
  <div>
    {portfolioData[dataIndex].data.map((item, index) => (
      <div key={index} className="mb-8">
